...
```{r daily-count-novel-cases-continent, fig.height = 5, fig.width = 10}
...
daily_cases_select <- df %>%
  ...
  filter(country %in% c("China", "Italy", "US", "Germany")) %>%
  arrange(country, date) %>%
  group_by(country) %>%
  mutate(new_cases = confirmed - lag(confirmed)) %>%
  ungroup() %>%
...
It's not really fair to compare absolute numbers between countries since populations vary so much. In the next code chunk I'll do some cursory examination of cases per capita (i.e. cases per 100,000 people).
...
Not looking great for Europe... but Europe has many small countries which may 'over-emphasise' (for lack of a better word) small numbers of cases. Let's sum all of Europe and compare it to the US 

```{r}
europe_pop <- correct_days %>% left_join(populations, by = "country_code") %>% filter(continent == "Europe") %>% distinct(country_code, pop) %>% summarise(sum(pop, na.rm = T)) %>% pull() - 145934462 # excluded Russia since they're playing silly buggers
america_pop <- correct_days %>% left_join(populations, by = "country_code") %>% filter(country_code == "US") %>% distinct(country_code, pop) %>% summarise(sum(pop, na.rm = T)) %>% pull()


correct_days %>%
  left_join(populations, by = "country_code") %>%
  filter((continent == "Americas" & country_code == "US") | (continent == "Europe")) %>%
  mutate(
    pop = case_when(
      continent == "Americas" ~ america_pop,
      continent == "Europe" ~ europe_pop
     ),
  # Lazy renaming..
  continent = if_else(continent == "Americas", "US", "Europe")) %>%
  group_by(continent, date, pop) %>%
  summarise(cases = sum(cases)) %>%
  ungroup() %>%
  mutate(per_capita = 100000 * (cases/pop)) %>%
  ggplot(aes(x = date, y = per_capita, group = continent, colour = continent)) +
  geom_line() +
  geom_point() +
  theme_minimal() +
  theme(
...
highlight_countries <- c("Italy", "Iran", "United Kingdom", "China", "US", "Spain", "Netherlands", "Sweden", "Germany")
...
county_maps <- read_sf(here('./data/sf_files/'))
...
map_seq_plot <- function(df, min_date, max_date, n_col, ext) {
  # Range of dates to plot
  date_seq <- seq(as.Date(min_date), as.Date(max_date), by = 1)

  # Compute lims for colour scale. Limit to values within date range.
  # Only select values > 0 to avoid error when log-scaling
  lims <- c(
  ...
  # Country map. This helps ensure proper alignment from plot to plot. Bit hacky.
  ...
  # Save plots
  ggsave(filename = glue("./output/{ min_date }-{ max_date }.{ ext }"), plot = plot_wrapped, height = 6.5, width = 10, unit = "in")
}
```
...
map_seq_plot(df = complete_df, min_date = min_date, max_date =max_date, n_col = 7, ext = "png")
...
ggsave(filename = glue("./output/{min(date_seq)}-{max(date_seq)}.{ ext }"), plot = plot_wrapped, height = 5, width = 10, unit = "in", dpi = 320)
```



```{r}
knitr::include_graphics(glue("./output/{min(date_seq)}-{max(date_seq)}.png"))
```
...
complete_df %>%
  filter(state_abb %in% c("IL", "WI", "MI", "IN")) %>%
  filter(date == max(date)) %>%
  ggplot() +
...
il_counties <- complete_df %>% as.data.frame() %>% filter(state == "Illinois") %>% distinct(COUNTYFP) %>% pull()

il_water <- 
  map_dfr(
    il_counties[-40],
    ~ area_water("IL", county = ., class = "sf") %>% filter(AWATER < 10^8)
  ) %>%
  summarise() %>%
  sf::st_as_sf()

  ggplot() +
    geom_sf(data = complete_df %>% filter(state == "Illinois"), aes(fill = per_capita)) +
    geom_sf(data = il_water, fill = "blue", colour = alpha("blue", 0.2)) +
  coord_sf(crs = 4326, 
           expand = TRUE) +
  scale_fill_continuous_sequential(
    palette = "Greens",
...
tigris::area_water("IL", "Cook") %>% 
  sf::st_as_sf() %>%
  ggplot() +
  geom_sf(alpha = 1, colour = alpha("blue", 0.2))
```
