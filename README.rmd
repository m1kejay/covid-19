...
daily_state_cases %>%
  arrange(state, dateChecked) %>%
  group_by(state) %>%
  mutate(daily_change = positive - lag(positive, 1)) %>%
  select(date, state, positive, daily_change, everything()) %>%
  ungroup() %>%
  filter(!is.na(state)) %>%
  mutate(protesting = if_else(state %in% c("Colorado", "Michigan"), "yes", "no"))
  ggplot(aes(x = dateChecked, y = daily_change)) +
